...
const CodePreview: React.FC<CodePreviewProps> = ({ pages, activePageFilename, onPageChange, activePageData }) => {
  ...
  const getCodeContent = () => {
    ...
    switch (activeCodeType) {
      case 'HTML': return activePageData.html;
      case 'CSS': return activePageData.css;
      case 'JS': return activePageData.js;
      default: return '';
    }
  };
  ...
  return (
    <div className="bg-white border-2 border-black flex flex-col text-sm">
      ...
      <div className="flex-shrink-0 p-2 flex items-center justify-between bg-stone-100 border-b-2 border-black">
          <div className="flex items-center gap-1">
              <CodeTypeButton type="HTML" activeType={activeCodeType} setType={setActiveCodeType}><HtmlIcon/></CodeTypeButton>
              <CodeTypeButton type="CSS" activeType={activeCodeType} setType={setActiveCodeType}><CssIcon/></CodeTypeButton>
              <CodeTypeButton type="JS" activeType={activeCodeType} setType={setActiveCodeType}><JsIcon/></CodeTypeButton>
          </div>
      </div>
...
const CodeTypeButton: React.FC<CodeTypeButtonProps> = ({ type, activeType, setType, children }) => {
    return (
        <button 
            onClick={() => setType(type)}
            className={`flex items-center gap-2 px-3 py-1.5 font-bold ${activeType === type ? 'bg-black text-white' : 'text-stone-600 hover:bg-stone-300'}`}
        >
            {children}
            {type}
        </button>
