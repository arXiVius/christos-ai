...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (generatedSite && generatedSite.length > 0) {
      const indexPage =
        generatedSite.find((p) => p.filename === 'index.html') ||
        generatedSite[0];
      ...
      >
          ...
          {!generatedSite ? (
            ...
            !isLoading && (
              <div className="text-left bg-stone-50 border-2 border-black p-4">
                <h2 className="font-bold uppercase tracking-wider text-sm text-stone-600">
                  Refine Vision
                </h2>
        ...
        {hasContent && (
          <div className="w-full max-w-7xl mx-auto flex-grow flex flex-col">
            {error && (
              <div
                className="bg-red-100 border-2 border-red-500 text-red-700 px-4 py-3 text-center my-4"
                role="alert"
              >
                {error}
              </div>
